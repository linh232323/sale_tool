...
class Visa extends Default_Controller {
    ...
    private function _configLink() {
        // Config some link
        $this->app_data['register_url'] = $this->app_base_url . $this->_default_module . 'visa/register';
        $this->app_data['forgot_url'] = $this->app_base_url . $this->_default_module . "visa/forgot";
        $this->app_data['resend_url'] = $this->app_base_url . $this->_default_module . "visa/resendEmail";
        $this->app_data['login_url'] = $this->app_base_url . $this->_default_module . "visa/login";
        $this->app_data['logout_url'] = $this->app_base_url . $this->_default_module . "visa/logout";
    }

    public function index() {
        $this->my_layout->setPageTitle('User Home');
        $this->my_layout->view('/user/login', $this->app_data);
    }
    ...
    public function register() {
        ...
        $this->form_validation->set_rules('FirstName', 'Họ', 'trim|required|xss_clean');
        $this->form_validation->set_rules('LastName', 'Tên', 'trim|required|xss_clean');
        $this->form_validation->set_rules('UserName', 'Username', 'trim|required|min_length[6]|max_length[25]|is_unique[w_customer.username]|xss_clean');
        $this->form_validation->set_rules('UserPasswd', 'mật khẩu', 'trim|required|min_length[6]|max_length[50]');
        $this->form_validation->set_rules('UserPasswd2', 'xác nhận mật khẩu', 'required|matches[UserPasswd]');
        $this->form_validation->set_rules('IdentifyNumber', 'Chứng minh thư, hộ chiếu', 'trim|required|numeric|min_length[8]|max_length[20]');
        $this->form_validation->set_rules('UserEmail', 'Email', 'trim|required|valid_email|is_unique[w_customer.email]');
        $this->form_validation->set_rules('CaptchaText', 'mã bảo vệ', 'trim|required|callback_validCaptcha');
        $this->form_validation->set_rules('UserAddress', 'Address', 'required');
        $this->form_validation->set_rules('UserPhone', 'Phone number', 'trim|required|numeric||min_length[6]|max_length[15]xss_clean');
        ...
        $this->form_validation->set_message('is_unique', '%s đã được sử dụng. Vui lòng chọn tên khác.');
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->my_layout->view('/user/visa', $this->app_data);
        } else {
            // Register New User, Send Validation Code to the Email
            $v_salt = random_string('alpha', 5);
            $a_insert_row = array(
                'firstname' => trim($this->input->post('FirstName')),
                'lastname' => trim($this->input->post('LastName')),
                'username' => trim($this->input->post('UserName')),
                'usersalt' => $v_salt,
                'password' => md5($this->input->post('UserPasswd') . $v_salt),
                'email' => trim($this->input->post('UserEmail')),
                'identify_number' => trim($this->input->post('IdentifyNumber')),
            ...
            if ($this->muser->add($a_insert_row)) {
                ...
                $v_sent = $this->_sendValidationEmail($a_insert_row['email']);
                $this->session->set_userdata($this->_sess_register, TRUE);
                redirect($this->app_base_url . $this->_default_module . 'visa/registered');
            } else {
    ...
    public function login() {
        ...
        if ($this->my_auth->isLogin()) {
            redirect($this->app_base_url . $this->_default_module . 'index');
        }
    ...
    private function _sendForgotPassEmail($email) {
        ...
        if ($a_user) {
            ...
            if ($this->muser->setRequestToken($a_user->UserId, $v_token)) {
                $this->app_data['reset_link'] = $this->app_base_url . 'default/visa/reset?email=' . $email . '&token=' . $v_token;
    ...
    private function _sendValidationEmail($email) {
        ...
        if ($a_user) {
            if ($a_user->userstatus != 'DEACTIVE') {
                ...
                //$this->email->initialize($config);
                $this->email->from('email@gmail.com', 'Test');
                $this->email->to($email);
                //$this->email->to('qxcuong@gmail.com');
